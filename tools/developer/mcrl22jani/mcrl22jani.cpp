...
class specification_basic_type1
{
public:
  ...
  specification_basic_type1(const specification_basic_type1&) = delete;
  specification_basic_type1& operator=(const specification_basic_type1&) = delete;
...
public:
  specification_basic_type1(const process::action_label_list& as,
      ...
      options(opt)
  {
    ...
    terminationAction
      = action(action_label(fresh_identifier_generator("Terminate"), sort_expression_list()), data_expression_list());
    terminatedProcId = process_identifier(fresh_identifier_generator("Terminated**"), variable_list());
    // /* Changed delta() to DeltaAtZero on 24/12/2006. Moved back in spring 2007, as this introduces unwanted time
    // constraints. */
    insert_process_declaration(terminatedProcId, variable_list(), seq(terminationAction, delta()), pCRL, false, false);
    delta_process = newprocess(variable_list(), delta(), pCRL, false, false);
    tau_process = newprocess(variable_list(), tau(), pCRL, true, false);
  }

  ~specification_basic_type1()
  {
...
private:
  ...
  static process_expression delta_at_zero() { return at(delta(), sort_real::real_(0)); }
  ...
  {
    return process::replace_variables_capture_avoiding_with_an_identifier_generator(e,
      sigma,
  ...
  void addString(const identifier_string& str) { fresh_identifier_generator.add_identifier(str); }
  ...
  static action_list to_sorted_action_list(const process_expression& p) { return sort_actions(to_action_list(p)); }
  ...
  objectdatatype& addMultiAction(const process_expression& multiAction, bool& isnew)
  {
    ...
    if (isnew)
    {
      ...
      // multiaction.
      const action_list tempvar
        = makemultiaction(actionnames, variable_list_to_data_expression_list(object.parameters));
  ...
  action RewriteAction(const action& t) { return action(t.label(), RewriteTermList(t.arguments())); }
  ...
  process_expression pCRLrewrite(const process_expression& t)
  {
    ...
    if (is_seq(t))
    {
      ...
      return seq(pCRLrewrite(seq(t).left()), pCRLrewrite(seq(t).right()));
    }
  ...
  objectdatatype& insertAction(const action_label& actionId)
  {
    ...
    {
      throw mcrl2::runtime_error("Action " + process::pp(actionId)
  ...
  objectdatatype& insert_process_declaration(const process_identifier& procId,
    ...
    const bool containstime)
  {
    ...
    {
      throw mcrl2::runtime_error("Process " + process::pp(procId)
                                 + " is added twice. This is an internal error in the lineariser. Please report. ");
    }
  ...
  void storeprocs(const std::vector<process_equation>& procs)
  {
    ...
    {
      insert_process_declaration(e.identifier(), e.formal_parameters(), e.expression(), unknown, false, false);
    }
  }

  bool searchProcDeclaration(const variable_list& parameters,
    const process_expression& body,
    ...
    process_identifier& p) const
  {
    ...
    {
      if (d.second.object == proc && d.second.parameters == parameters && d.second.processbody == body
          && d.second.canterminate == canterminate && d.second.containstime == containstime
...
private:
  ...
  processstatustype determine_process_statusterm(const process_expression& body, // intentionally not a reference.
    const processstatustype status)
  {
    ...
    if (is_choice(body))
    {
      ...
      if ((s1 == mCRL) || (s2 == mCRL))
      {
        throw mcrl2::runtime_error(
          "An operator ||, allow, hide, rename, or comm occurs within the scope of a choice operator in "
    ...
    if (is_seq(body))
    {
      ...
      {
        throw mcrl2::runtime_error(
          "An operator ||, allow, hide, rename, or comm occurs in the scope of a sequential operator in "
          + process::pp(body) + ". " + "The lineariser cannot handle such processes. ");
      }
      return pCRL;
    }
    ...
    if (is_if_then(body))
    {
      ...
      {
        throw mcrl2::runtime_error(
          "If-then occurs in a multi-action in " + process::pp(body) + ". The lineariser cannot linearise this. ");
      }
      const processstatustype s1 = determine_process_statusterm(if_then(body).then_case(), pCRL);
      if (s1 == mCRL)
      {
        throw mcrl2::runtime_error(
          "An operator ||, allow, hide, rename, or comm occurs in the scope of the if-then operator in "
          + process::pp(body) + ". " + "The lineariser cannot handle such processes. ");
      }
      return pCRL;
    }
    ...
    if (is_if_then_else(body))
    {
      ...
      {
        throw mcrl2::runtime_error(
          "An operator ||, allow, hide, rename, or comm occurs in the scope of the if-then-else operator in "
          + process::pp(body) + ". " + "The lineariser cannot handle such processes. ");
      }
      return pCRL;
    }
    ...
    if (is_stochastic_operator(body))
    {
      ...
      {
        throw mcrl2::runtime_error(
          "An operator ||, allow, hide, rename, or comm occurs in the scope of the stochastic operator in "
          + process::pp(body) + ". " + "The lineariser cannot handle such processes. ");
      }
      return s1;
    }
    ...
    if (is_comm(body))
    {
      if (status != mCRL)
      {
        throw mcrl2::runtime_error("The communication operator occurs in the scope of recursion, the condition, the "
                                   "sequential operation or the choice in "
    ...
    if (is_hide(body))
    {
      ...
      {
        throw mcrl2::runtime_error(
          "A hide operator occurs in the scope of recursion, or a condition, choice or sequential operator in "
          + process::pp(body) + ".");
      }
    ...
    if (is_rename(body))
    {
      ...
      {
        throw mcrl2::runtime_error(
          "A rename operator occurs in the scope of recursion, or a condition, choice or sequential operator in "
          + process::pp(body) + ".");
      }
    ...
    if (is_allow(body))
    {
      ...
      {
        throw mcrl2::runtime_error(
          "An allow operator occurs in the scope of recursion, or a condition, choice or sequential operator in "
          + process::pp(body) + ".");
      }
    ...
    if (is_block(body))
    {
      ...
      {
        throw mcrl2::runtime_error(
          "A block operator occurs in the scope of recursion, or a condition, choice or sequential operator in "
          + process::pp(body) + ".");
      }
...
public:
    ...
    std::set<process_identifier>& visited)
  {
    ...
    if ((is_delta(body)) || (is_tau(body)) || (is_action(body)))
    {
      return;
    }
  ...
  void collectPcrlProcesses(const process_identifier& procDecl,
    std::vector<process_identifier>& pcrlprocesses,
    std::set<process_identifier>& visited)
  {
...
public:
  ...
  void
  filter_vars_by_term(const data_expression& t, const std::set<variable>& vars_set, std::set<variable>& vars_result_set)
  {
    if (is_variable(t))
    {
  ...
  void filter_vars_by_termlist(Iterator begin,
    const Iterator& end,
  ...
  void filter_vars_by_multiaction(const action_list& multiaction,
    const std::set<variable>& vars_set,
    std::set<variable>& vars_result_set)
  {
  ...
  void filter_vars_by_assignmentlist(const assignment_list& assignments,
    const variable_list& parameters,
    const std::set<variable>& vars_set,
    std::set<variable>& vars_result_set)
  {
    const auto& l
      = parameters | std::views::transform([](const variable& v) { return atermpp::down_cast<data_expression>(v); });
  ...
  bool occursinpCRLterm(const variable& var, const process_expression& p, const bool strict)
  {
    if (is_choice(p))
    {
      return occursinpCRLterm(var, choice(p).left(), strict) || occursinpCRLterm(var, choice(p).right(), strict);
    }
    if (is_seq(p))
    {
      return occursinpCRLterm(var, seq(p).left(), strict) || occursinpCRLterm(var, seq(p).right(), strict);
    }
    if (is_if_then(p))
    {
      return occursinterm(if_then(p).condition(), var) || occursinpCRLterm(var, if_then(p).then_case(), strict);
    }
    ...
    if (is_stochastic_operator(p))
    {
      ...
      if (strict)
      {
        return occursintermlist(var, variable_list_to_data_expression_list(sto.variables()))
               || occursinterm(sto.distribution(), var) || occursinpCRLterm(var, sto.operand(), strict);
      }
      else
      {
        return (!occursintermlist(var, variable_list_to_data_expression_list(sto.variables())))
    ...
    {
      return occursintermlist(var,
        process_instance_assignment(p).assignments(),
        process_instance_assignment(p).identifier());
    }
    if (is_action(p))
    {
    ...
    if (is_at(p))
    {
      return occursinterm(at(p).time_stamp(), var) || occursinpCRLterm(var, at(p).operand(), strict);
    }
    if (is_delta(p))
    {
      return false;
  ...
  template<class MutableSubstitution>
  void alphaconvert(variable_list& sumvars,
    ...
    const data_expression_list& occurterms)
  {
    ...
    for (const variable& var: sumvars)
    {
      if (occursintermlist(var, variable_list_to_data_expression_list(occurvars)) || occursintermlist(var, occurterms))
      {
        const variable newvar = get_fresh_variable(var.name(), var.sort());
  ...
  template<class Substitution>
  assignment_list substitute_assignmentlist(const assignment_list& assignments,
    const variable_list& parameters,
    ...
    Substitution& sigma)
  {
    ...
    */
    ...
    if (!assignments.empty())
    {
      ...
      if (parameter == lhs)
      {
        ...
        assignment_list result
          = substitute_assignmentlist(assignments.tail(), parameters.tail(), replacelhs, replacerhs, sigma);
    ...
    if (lhs == rhs)
    {
      return substitute_assignmentlist(assignments, parameters.tail(), replacelhs, replacerhs, sigma);
    }
    assignment_list result = substitute_assignmentlist(assignments, parameters.tail(), replacelhs, replacerhs, sigma);
    result.push_front(assignment(lhs, rhs));
    return result;
  }
  ...
  process_expression substitute_pCRLproc(const process_expression& p, Substitution& sigma)
  {
    ...
    if (is_if_then_else(p))
    {
      data_expression condition
        = RewriteTerm(replace_variables_capture_avoiding_alt(if_then_else(p).condition(), sigma));
      if (condition == sort_bool::false_())
      {
    ...
    if (is_sum(p))
    {
      ...
      return sum(sumargs, substitute_pCRLproc(sum(p).operand(), local_sigma));
    }
    ...
    if (is_stochastic_operator(p))
    {
      ...
      return stochastic_operator(sumargs,
    ...
    if (is_process_instance_assignment(p))
    {
      ...
      const assignment_list new_assignments
        = substitute_assignmentlist(q.assignments(), parameters, false, true, sigma);
    ...
    if (is_action(p))
    {
      return action(action(p).label(), replace_variables_capture_avoiding_alt(action(p).arguments(), sigma));
    }
    ...
    if (is_sync(p))
    {
      return process::sync(substitute_pCRLproc(process::sync(p).left(), sigma),
    ...
    const std::set<variable>& bound_variables = std::set<variable>())
  {
    ...
    assert(check_valid_process_instance_assignment(procId.identifier(),
      assignment_list(new_assignments.begin(), new_assignments.end())));
  ...
  process_identifier newprocess(const variable_list& parameters,
    const process_expression& body,
    ...
    const bool containstime)
  {
    ...
    if (numberOfNewProcesses == warningNumber)
    {
      ...
      if (options.lin_method == lmRegular)
      {
        mCRL2log(mcrl2::log::warning)
          << " A possible unbounded loop can be avoided by using `regular2' or `stack' as linearisation method."
          << std::endl;
      }
    ...
    insert_process_declaration(p, parameters1, body, ps, canterminate, containstime);
    return p;
  }

  process_expression
  wraptime(const process_expression& body, const data_expression& time, const variable_list& freevars)
  {
    if (is_choice(body))
    {
      return choice(wraptime(choice(body).left(), time, freevars), wraptime(choice(body).right(), time, freevars));
    }
    ...
    {
      /* make a new process */
      const process_identifier newproc
        = newprocess(freevars, body, pCRL, canterminatebody(body), containstimebody(body));
      assert(check_valid_process_instance_assignment(newproc, assignment_list()));
      return at(process_instance_assignment(newproc,
                  assignment_list()), // data::data_expression_list(objectdata[objectIndex(newproc)].parameters)),
        time);
    }

    if ( // (is_process_instance(body))||
      (is_process_instance_assignment(body)) || (is_sync(body)) || (is_action(body)) || (is_tau(body))
      || (is_delta(body)))
    {
      return at(body, time);
    }
  ...
  process_expression distributeActionOverConditions(const process_expression& act, // This is a multi-action, actually.
    ...
    const std::set<variable>& variables_bound_in_sum)
  {
    if (is_if_then(restterm))
    {
      ...
      const process_expression r = choice(distributeActionOverConditions(act,
                                            lazy::and_(condition, c),
                                            if_then(restterm).then_case(),
                                            freevars,
                                            variables_bound_in_sum),
        distributeActionOverConditions(act,
  ...
  static assignment_list parameters_to_assignment_list(const variable_list& parameters,
    const std::set<variable>& variables_bound_in_sum)
  {
    assignment_vector result;
    for (const variable& v: parameters)
  ...
  process_expression bodytovarheadGNF(const process_expression& body,
    const state s,
    const variable_list& freevars,
    ...
    const std::set<variable>& variables_bound_in_sum)
  {
    ...
    if (is_choice(body))
    {
      if (alt_state >= s)
      {
        const process_expression body1
          = bodytovarheadGNF(choice(body).left(), alt_state, freevars, first, variables_bound_in_sum);
        const process_expression body2
          = bodytovarheadGNF(choice(body).right(), alt_state, freevars, first, variables_bound_in_sum);
      ...
      const process_identifier newproc
        = newprocess(freevars, body1, pCRL, canterminatebody(body1), containstimebody(body1));
      assert(check_valid_process_instance_assignment(newproc,
        parameters_to_assignment_list(objectIndex(newproc).parameters, variables_bound_in_sum)));
      return process_instance_assignment(newproc,
        parameters_to_assignment_list(objectIndex(newproc).parameters, variables_bound_in_sum));
    }
    ...
    if (is_sum(body))
    {
      if (sum_state >= s)
      {
        ...
        if (is_choice(body1))
        {
          return choice(sum(sumvars, choice(body1).left()), sum(sumvars, choice(body1).right()));
        }
        return sum(sumvars, body1);
      }
      const process_expression body1 = bodytovarheadGNF(body, alt_state, freevars, first, variables_bound_in_sum);
    ...
    if (is_stochastic_operator(body))
    {
      ...
      const process_identifier newproc
        = newprocess(freevars, body_, pCRL, canterminatebody(body_), containstimebody(body_));
      assert(check_valid_process_instance_assignment(newproc,
        parameters_to_assignment_list(objectIndex(newproc).parameters, variables_bound_in_sum)));
      return process_instance_assignment(newproc,
        parameters_to_assignment_list(objectIndex(newproc).parameters, variables_bound_in_sum));
    ...
    if (is_if_then(body))
    {
      ...
      if (s <= sum_state)
      {
        return if_then(condition, bodytovarheadGNF(body1, seq_state, freevars, first, variables_bound_in_sum));
      }
      const process_expression body2 = bodytovarheadGNF(body, alt_state, freevars, first, variables_bound_in_sum);
      const process_identifier newproc
        = newprocess(freevars, body2, pCRL, canterminatebody(body2), containstimebody(body2));
      assert(check_valid_process_instance_assignment(newproc,
        parameters_to_assignment_list(objectIndex(newproc).parameters, variables_bound_in_sum)));
      return process_instance_assignment(newproc,
        parameters_to_assignment_list(objectIndex(newproc).parameters, variables_bound_in_sum));
    ...
    if (is_if_then_else(body))
    {
      ...
      if ((s <= sum_state) && ((isDeltaAtZero(body1)) || (isDeltaAtZero(body2))))
      {
        ...
        /* body1=="Delta@0" */
        {
          return if_then(lazy::not_(condition),
            bodytovarheadGNF(body2, seq_state, freevars, first, variables_bound_in_sum));
        }
      }
      if (alt_state == s) /* body1!=Delta@0 and body2!=Delta@0 */
      {
        return choice(if_then(condition, bodytovarheadGNF(body1, seq_state, freevars, first, variables_bound_in_sum)),
          if_then(lazy::not_(condition), bodytovarheadGNF(body2, seq_state, freevars, first, variables_bound_in_sum)));
      }
      const process_expression body3 = bodytovarheadGNF(body, alt_state, freevars, first, variables_bound_in_sum);
      const process_identifier newproc
        = newprocess(freevars, body3, pCRL, canterminatebody(body3), containstimebody(body3));
      assert(check_valid_process_instance_assignment(newproc,
        parameters_to_assignment_list(objectIndex(newproc).parameters, variables_bound_in_sum)));
      return process_instance_assignment(newproc,
        parameters_to_assignment_list(objectIndex(newproc).parameters, variables_bound_in_sum));
      ...
      if (s <= seq_state)
      {
        ...
        if ((is_if_then_else(body2)) && (s <= sum_state))
        {
          ...
          const process_expression r = choice(
            ...
            distributeActionOverConditions(body1,
              lazy::not_(c),
              if_then_else(body2).else_case(),
              freevars,
              variables_bound_in_sum));
          return r;
        }
    ...
    if (is_action(body))
    {
      ...
      if (object.process_representing_action == process_identifier())
      {
        /* this action does not yet have a corresponding process, which
           ...
           of newprocess */
        const process_identifier tempvar = newprocess(object.parameters, object.processbody, GNF, true, false);
        object.process_representing_action = tempvar;
      }
      return transform_process_instance_to_process_instance_assignment(
        process_instance(object.process_representing_action, action(body).arguments()));
    }
    ...
    if (is_sync(body))
    {
      ...
      if (object.process_representing_action == process_identifier())
      {
        ...
        process_identifier tempvar = newprocess(object.parameters, object.processbody, GNF, true, false);
        object.process_representing_action = tempvar;
      }
      return transform_process_instance_to_process_instance_assignment(
        process_instance(process_identifier(object.process_representing_action), getarguments(ma)));
    }

    if (is_at(body))
    {
      process_expression body1 = bodytovarheadGNF(at(body).operand(), s, freevars, first, variables_bound_in_sum);
  ...
  void procstovarheadGNF(const std::vector<process_identifier>& procs)
  {
    ...
    for (const process_identifier& i: procs)
    {
      ...
      const process_expression result
        = bodytovarheadGNF(object.processbody, alt_state, object.parameters, first, variables_bound_in_sum);
  ...
  process_expression putbehind(const process_expression& body1, const process_expression& body2)
  {
    if (is_choice(body1))
    {
      return choice(putbehind(choice(body1).left(), body2), putbehind(choice(body1).right(), body2));
    }
    ...
    if (is_sum(body1))
    {
      ...
      return sum(sumvars, putbehind(substitute_pCRLproc(sum(body1).operand(), sigma), body2));
    }
    ...
    if (is_stochastic_operator(body1))
    {
      ...
      return stochastic_operator(stochvars,
        sto.distribution(),
        putbehind(substitute_pCRLproc(sto.operand(), sigma), body2));
    }
  ...
  process_expression distribute_condition(const process_expression& body1, const data_expression& condition)
  {
    if (is_choice(body1))
    {
      return choice(distribute_condition(choice(body1).left(), condition),
    ...
    if (is_if_then(body1))
    {
      return if_then(lazy::and_(if_then(body1).condition(), condition), if_then(body1).then_case());
    }
    ...
    if (is_sum(body1))
    {
      ...
      return sum(sumvars, distribute_condition(substitute_pCRLproc(sum(body1).operand(), sigma), condition));
    }
    ...
    if (is_stochastic_operator(body1))
    {
      ...
      return stochastic_operator(stochvars,
        ...
        distribute_condition(substitute_pCRLproc(sto.operand(), sigma), condition));
    }

    if (is_at(body1) || is_action(body1) || is_sync(body1) || is_process_instance_assignment(body1) || is_delta(body1)
        || is_tau(body1))
    {
      return if_then(condition, body1);
    }
  ...
  */
  ...
  process_expression enumerate_distribution_and_sums(const variable_list& sumvars,
    ...
    const process_expression& body)
  {
    ...
    for (const variable& v: sumvars)
    {
      ...
      {
        throw mcrl2::runtime_error("Cannot distribute a sum variable of non finite sort " + pp(v.sort())
  ...
  process_expression distribute_sum_over_a_stochastic_operator(const variable_list& sumvars,
    const variable_list& stochastic_variables,
    const data_expression& distribution,
    const process_expression& body)
  {
    if (is_sum(body) || is_choice(body) || is_seq(body) || is_if_then(body) || is_sync(body) || is_action(body)
        || is_tau(body) || is_at(body) || is_process_instance_assignment(body) || isDeltaAtZero(body))
    {
      return enumerate_distribution_and_sums(sumvars, stochastic_variables, distribution, body);
    }

    if (is_delta(body) || is_tau(body))
    {
      return body;
    }
    ...
    if (is_stochastic_operator(body))
    {
      ...
      return distribute_sum_over_a_stochastic_operator(sumvars,
        ...
        sort_real::times(distribution, new_distribution),
        new_body);
    }
  ...
  process_expression distribute_sum(const variable_list& sumvars, const process_expression& body1)
  {
    if (is_choice(body1))
    {
      return choice(distribute_sum(sumvars, choice(body1).left()), distribute_sum(sumvars, choice(body1).right()));
    }

    if (is_seq(body1) || is_if_then(body1) || is_sync(body1) || is_action(body1) || is_tau(body1) || is_at(body1)
        || is_process_instance_assignment(body1) || isDeltaAtZero(body1))
    {
      return sum(sumvars, body1);
    }
    ...
    if (is_delta(body1) || is_tau(body1))
    {
      return body1;
    }
  ...
  static int match_sequence(const std::vector<process_instance_assignment>& s1,
    ...
    const bool regular2)
  {
    ...
    for (std::vector<process_instance_assignment>::const_iterator i1 = s1.begin(); i1 != s1.end(); ++i1, ++i2)
    {
      if (i2 == s2.end())
      {
        return false;
  ...
  bool exists_variable_for_sequence(const std::vector<process_instance_assignment>& process_names,
    process_identifier& result)
  {
    ...
    for (std::vector<process_identifier>::const_iterator walker = seq_varnames.begin(); walker != seq_varnames.end();
      ++walker, ++rwalker)
    {
      ...
      if (is_process_instance_assignment(first))
      {
        ...
        if (object.canterminate)
        {
          ...
          assert(check_valid_process_instance_assignment(procId,
            assignment_list(new_assignment.begin(), new_assignment.end())));
          newbody
            = seq(process_instance_assignment(procId, assignment_list(new_assignment.begin(), new_assignment.end())),
              newbody);
          const variable_list result = pars1 + pars;
          assert(std::set<variable>(result.begin(), result.end()).size()
                 == result.size()); // all elements in the result are unique.
          return result;
        }
        else
        {
  ...
  assignment_list argscollect_regular(const process_expression& t,
  ...
  process_expression create_regular_invocation(process_expression sequence,
    ...
    const std::set<variable>& variables_bound_in_sum)
  {
    ...
    if (!exists_variable_for_sequence(process_names, new_process))
    {
      ...
      if (options.lin_method == lmRegular2)
      {
        ...
        new_process = newprocess(pars, newbody, pCRL, canterminatebody(newbody), containstimebody(newbody));
        representedprocesses.push_back(process_names);
      }
      else
      {
        new_process = newprocess(freevars, sequence, pCRL, canterminatebody(sequence), containstimebody(sequence));
        representedprocesses.push_back(process_names);
      }
    ...
    {
      assert(check_valid_process_instance_assignment(new_process,
        argscollect_regular(sequence, parameters, variables_bound_in_sum)));
      return process_instance_assignment(new_process,
        argscollect_regular(sequence, parameters, variables_bound_in_sum));
    }
  }

  process_expression to_regular_form(const process_expression& t,
    std::vector<process_identifier>& todo,
    const variable_list& freevars,
    const std::set<variable>& variables_bound_in_sum)
     ...
     on the todo list, to be transformed to regular form also. */
  {
    ...
    if (is_if_then(t))
    {
      return if_then(if_then(t).condition(),
        to_regular_form(if_then(t).then_case(), todo, freevars, variables_bound_in_sum));
    }
    ...
    if (is_sum(t))
    {
      ...
      std::set<variable> variables_bound_in_sum1 = variables_bound_in_sum;
      variables_bound_in_sum1.insert(sumvars.begin(), sumvars.end());
      return sum(sumvars, to_regular_form(body, todo, sumvars + freevars, variables_bound_in_sum1));
    }
    ...
    if (is_stochastic_operator(t))
    {
      ...
      maintain_variables_in_rhs<mutable_map_substitution<>> sigma;
      alphaconvert(sumvars, sigma, freevars, data_expression_list());
      const data_expression distribution = replace_variables_capture_avoiding_alt(sto.distribution(), sigma);
      const process_expression body = substitute_pCRLproc(sto.operand(), sigma);

      std::set<variable> variables_bound_in_sum1 = variables_bound_in_sum;
      variables_bound_in_sum1.insert(sumvars.begin(), sumvars.end());
      return stochastic_operator(sumvars,
        distribution,
        to_regular_form(body, todo, sumvars + freevars, variables_bound_in_sum1));
    }
  ...
  process_expression distributeTime(const process_expression& body,
    ...
    data_expression& timecondition)
  {
    if (is_choice(body))
    {
      return choice(distributeTime(choice(body).left(), time, freevars, timecondition),
    ...
    if (is_if_then(body))
    {
      ...
      process_expression body1 = distributeTime(if_then(body).then_case(), time, freevars, timecondition);

      return if_then(lazy::and_(data_expression(if_then(body).condition()), timecondition), body1);
    }

    if (is_seq(body))
    {
      return seq(distributeTime(seq(body).left(), time, freevars, timecondition), seq(body).right());
    }
    ...
    if ((is_sync(body)) || (is_action(body)) || (is_tau(body)) || (is_delta(body)))
    {
      return at(body, time);
    }
  ...
  process_expression procstorealGNFbody(const process_expression& body,
    variableposition v,
    std::vector<process_identifier>& todo,
    const bool regular,
     ...
     variable. */
  {
    ...
    {
      data_expression timecondition = sort_bool::true_();
      process_expression body1
        = procstorealGNFbody(at(body).operand(), first, todo, regular, mode, freevars, variables_bound_in_sum);
      return distributeTime(body1, at(body).time_stamp(), freevars, timecondition);
    }

    if (is_choice(body))
    {
      const process_expression body1
        = procstorealGNFbody(choice(body).left(), first, todo, regular, mode, freevars, variables_bound_in_sum);
      const process_expression body2
        = procstorealGNFbody(choice(body).right(), first, todo, regular, mode, freevars, variables_bound_in_sum);
      return choice(body1, body2);
    }
    ...
    {
      const process_expression body1
        = procstorealGNFbody(seq(body).left(), v, todo, regular, mode, freevars, variables_bound_in_sum);
      const process_expression body2
        = procstorealGNFbody(seq(body).right(), later, todo, regular, mode, freevars, variables_bound_in_sum);
    ...
    if (is_if_then(body))
    {
      const process_expression r = distribute_condition(
        procstorealGNFbody(if_then(body).then_case(), first, todo, regular, mode, freevars, variables_bound_in_sum),
    ...
    if (is_sum(body))
    {
      ...
      return distribute_sum(sumvars,
        procstorealGNFbody(sum(body).operand(),
          first,
          todo,
          regular,
          mode,
    ...
    if (is_stochastic_operator(body))
    {
      ...
      return stochastic_operator(sumvars,
        sto.distribution(),
        procstorealGNFbody(sto.operand(), v, todo, regular, mode, sumvars + freevars, variables_bound_in_sum1));
    }
    ...
    if (is_merge(body))
    {
      procstorealGNFbody(process::merge(body).left(), later, todo, regular, mode, freevars, variables_bound_in_sum);
      procstorealGNFbody(process::merge(body).right(), later, todo, regular, mode, freevars, variables_bound_in_sum);
      return process_expression();
    }
  ...
  void procstorealGNFrec(const process_identifier& procIdDecl,
    const variableposition v,
    std::vector<process_identifier>& todo,
    const bool regular)
...

  {
    ...
    if (object.processstatus == pCRL)
    {
      ...
      const process_expression t
        = procstorealGNFbody(object.processbody, first, todo, regular, pCRL, object.parameters, variables_bound_in_sum);
    ...
    if (object.processstatus == mCRL)
    {
      ...
      std::set<variable> variables_bound_in_sum;
      const process_expression t
        = procstorealGNFbody(object.processbody, first, todo, regular, mCRL, object.parameters, variables_bound_in_sum);
    ...
    if ((object.processstatus == GNFbusy) || (object.processstatus == GNF) || (object.processstatus == mCRLdone)
  ...
  /* set_proc_identifier_map is an auxiliary procedure that effectively sets identfier_identifier_map[id1]:=... or
     identifier_identifier_map[id1]:=... in such a way that they map to the same element. In this way
     identifier_identifier_map acts as a representation of the equivalence set induced by equations id1==id2. A
     constraint is that initial_process may never occur as a left hand side. An invariant of identifier_identifier_map
     is that no id1 that occurs at the right of the map can occur at the left.
  */

  static process_identifier get_last(const process_identifier& id,
    const std::map<process_identifier, process_identifier>& identifier_identifier_map)
  {
    ...
    do
    {
      const std::map<process_identifier, process_identifier>::const_iterator i
        = identifier_identifier_map.find(target_id);
  ...
  static void set_proc_identifier_map(std::map<process_identifier, process_identifier>& identifier_identifier_map,
  ...
  std::set<process_identifier> minimize_set_of_reachable_process_identifiers(
  ...
  public:
    ...
    const process_expression& process_body() const { return m_process_body; }

    const process_identifier& process_id() const { return m_pid; }
  ...
  std::set<process_identifier> remove_stochastic_operators_from_front(
    const std::set<process_identifier>& reachable_process_identifiers,
    ...
    stochastic_distribution& initial_stochastic_distribution)
  {
    ...
    for (const process_identifier& p: reachable_process_identifiers)
    {
      ...
      if (!is_stochastic_operator(proc_))
      {
        processes_with_stochastic_distribution_first.insert(
          std::pair<process_identifier, process_pid_pair>(p, process_pid_pair(proc_, p)));
        result.insert(p);
      }
      else
      {
        ...
        const process_identifier newproc = newprocess(vars + object.parameters,
        ...
        // body the process.
        processes_with_stochastic_distribution_first.insert(std::pair<process_identifier, process_pid_pair>(p,
          process_pid_pair(stochastic_operator(vars,
    ...
    for (const process_identifier& p: reachable_process_identifiers)
    {
      ...
      object.processbody
        = transform_initial_distribution_term(object.processbody, processes_with_stochastic_distribution_first);
      assert(!is_stochastic_operator(object.processbody));
    }

    // Adapt the initial process
    process_expression initial_distribution
      = processes_with_stochastic_distribution_first.at(initial_process_id).process_body();
  ...
  void declare_control_state(const std::set<process_identifier>& pCRLprocs) { create_enumeratedtype(pCRLprocs.size()); }
...
private:
  ...
  public:
    ...
    stackoperations(stackoperations&&) = delete;
    stackoperations& operator=(stackoperations&&) = delete;
    ...
    stackoperations(const variable_list& pl, specification_basic_type1& spec)
    {
      parameter_list = pl;
      ...
      structured_sort_constructor sc_emptystack(spec.fresh_identifier_generator("emptystack"),
        spec.fresh_identifier_generator("isempty"));
      ...
      const std::vector<data::function_symbol> projection_functions = sc_push.projection_functions(stack_sort_alias);
  ...
  public:
    ...
    static stackoperations* find_suitable_stack_operations(const variable_list& parameters,
      stackoperations* stack_operations_list)
    {
    ...
    stacklisttype(const variable_list& parlist,
      specification_basic_type1& spec,
      const bool regular,
  ...
  data_expression getvar(const variable& var, const stacklisttype& stack) const
  {
    ...
    for (variable_list::const_iterator walker = stack.parameters.begin(); walker != stack.parameters.end();
      ++walker, ++getmappings)
    {
      if (*walker == var)
      {
  ...
  data_expression correctstatecond(const process_identifier& procId,
    ...
    int regular)
  {
    ...
    if (!options.newstate)
    {
      ...
      return equal_to(application(stack.opns->getstate, stack.stackvar),
  ...
  data_expression adapt_term_to_stack(const data_expression& t,
    const stacklisttype& stack,
    ...
    const variable_list& stochastic_variables)
  {
    ...
    if (is_variable(t))
    {
      ...
      else if (std::find(stochastic_variables.begin(), stochastic_variables.end(), t) != stochastic_variables.end())
      {
    ...
    if (is_application(t))
    {
      ...
      return application(adapt_term_to_stack(a.head(), stack, vars, stochastic_variables),
    ...
    if (is_abstraction(t))
    {
      ...
      return abstraction(abs_t.binding_operator(),
        abs_t.variables(),
    ...
    if (is_where_clause(t))
    {
      ...
      for (const assignment& a: old_assignments)
      {
        ...
        new_assignments.push_front(
          assignment(a.lhs(), adapt_term_to_stack(a.rhs(), stack, vars, stochastic_variables)));
      }
      return where_clause(adapt_term_to_stack(where_t, stack, new_vars, stochastic_variables), new_assignments);
    }
  ...
  template<typename Iterator>
  data_expression_vector adapt_termlist_to_stack(Iterator begin,
    const Iterator& end,
    const stacklisttype& stack,
    const variable_list& vars,
    const variable_list& stochastic_variables)
  ...
  data_expression find_(const variable& s,
  ...
  data_expression_list findarguments(const variable_list& pars,
    ...
    const variable_list& stochastic_variables)
  {
    ...
    data_expression_list result
      = findarguments(pars, parlist.tail(), args, t2, stack, vars, free_variables_in_body, stochastic_variables);
  ...
  assignment_list find_dummy_arguments(const variable_list& parlist, // The list of all parameters.
    const assignment_list& args,
    const std::set<variable>& free_variables_in_body,
    const variable_list& stochastic_variables)
  {
  ...
  assignment_list push_regular(const process_identifier& procId,
    const assignment_list& args,
    const stacklisttype& stack,
    const std::set<process_identifier>& pCRLprocs,
    bool singlestate,
    const variable_list& stochastic_variables)
  {
    ...
    const assignment_list t
      = find_dummy_arguments(stack.parameters, args, get_free_variables(object), stochastic_variables);
  ...
  assignment_list make_procargs_regular(const process_expression& t,
    const stacklisttype& stack,
    ...
    const variable_list& stochastic_variables)
  {
    ...
    if (is_process_instance_assignment(t))
    {
      ...
      return push_regular(procId, t1, stack, pcrlprcs, singlestate, stochastic_variables);
    }

    throw mcrl2::runtime_error("Expect seq or name constructing a recursive invocation: " + process::pp(t) + ".");
  }

  data_expression push_stack(const process_identifier& procId,
    const assignment_list& args,
    const data_expression_list& t2,
    const stacklisttype& stack,
    const std::set<process_identifier>& pCRLprocs,
  ...
  data_expression make_procargs_stack(const process_expression& t,
    const stacklisttype& stack,
    const std::set<process_identifier>& pcrlprcs,
    const variable_list& vars,
    const variable_list& stochastic_variables)
  ...
  assignment_list make_procargs(const process_expression& t,
    const stacklisttype& stack,
    const std::set<process_identifier>& pcrlprcs,
    const variable_list& vars,
    const bool regular,
  ...
  assignment_list
  pushdummy_regular(const variable_list& pars, const stacklisttype& stack, const variable_list& stochastic_variables)
  {
    /* stack.parameters is the total list of parameters of the
       aggregated pCRL process. The variable pars contains
       the list of all variables occuring in the initial
  ...
  data_expression_list pushdummyrec_stack(const variable_list& totalpars,
  ...
  data_expression_list pushdummy_stack(const variable_list& parameters,
    const stacklisttype& stack,
    const variable_list& stochastic_variables)
  {
  ...
  data_expression_list make_initialstate(const process_identifier& initialProcId,
    const stacklisttype& stack,
    const std::set<process_identifier>& pcrlprcs,
    const bool regular,
    const bool singlecontrolstate,
    const stochastic_distribution& initial_stochastic_distribution)
  {
    ...
    if (regular)
    {
      data_expression_list result = pushdummy_regular_data_expressions(objectIndex(initialProcId).parameters, stack);
      if (!singlecontrolstate)
      {
    ...
    {
      data_expression_list result
        = pushdummy_stack(objectIndex(initialProcId).parameters, stack, initial_stochastic_distribution.variables());
  ...
  void insert_summand(stochastic_action_summand_vector& action_summands,
    ...
    const bool is_deadlock_summand)
  {
    ...
    if (is_deadlock_summand)
    {
      insert_timed_delta_summand(action_summands,
        deadlock_summands,
        deadlock_summand(sumvars, rewritten_condition, has_time ? deadlock(actTime) : deadlock()),
        options.ignore_time);
    }
    else
    {
  ...
  void add_summands(const process_identifier& procId,
...

  {
    ...
    while ((is_if_then(summandterm) || is_stochastic_operator(summandterm)))
    {
        ...
        if (!(regular && singlestate))
        {
          condition1 = lazy::and_(condition1,
            ((regular) ? localcondition
                       : adapt_term_to_stack(localcondition, stack, sumvars, cumulative_distribution.variables())));
        }
        else
        {
      ...
      else
      {
        ...
        cumulative_distribution = stochastic_distribution(cumulative_distribution.variables() + sto.variables(),
          real_times_optimized(cumulative_distribution.distribution(), sto.distribution()));
    ...
    if (is_seq(summandterm))
    {
      ...
      const assignment_list procargs
        = make_procargs(t2_new, stack, pCRLprocs, sumvars, regular, singlestate, distribution.variables());
      ...
      if (!regular)
      {
        ...
        if (has_time)
        {
          atTime = adapt_term_to_stack(atTime, stack, sumvars, variable_list());
        }
        distribution = stochastic_distribution(distribution.variables(),
  ...
  void collectsumlistterm(const process_identifier& procId,
    stochastic_action_summand_vector& action_summands,
    deadlock_summand_vector& deadlock_summands,
    const process_expression& body,
    const variable_list& pars,
  ...
  void collectsumlist(stochastic_action_summand_vector& action_summands,
    deadlock_summand_vector& deadlock_summands,
    const std::set<process_identifier>& pCRLprocs,
    const variable_list& pars,
    const stacklisttype& stack,
  ...
  public:
    ...
    enumeratedtype(const std::size_t n, specification_basic_type1& spec)
    {
      size = n;
      if (n == 2)
      {
      ...
      else
      {
        ...
        for (std::size_t j = 0; (j < n); j++)
        {
          ...
          const identifier_string s
            = spec.fresh_identifier_generator("e" + std::to_string(j) + "_" + std::to_string(n));
  ...
  void define_equations_for_case_function(const std::size_t index,
    ...
    const sort_expression& sort)
  {
    ...
    data.add_equation(data_equation(variable_list({v1, v}), application(functionname, xxxterm), v1));
    ...
    for (const data_expression& w: elementnames)
    {
      ...
      args.push_front(w);
      data.add_equation(data_equation(vars, application(functionname, args), auxvars.front()));
  ...
  void create_case_function_on_enumeratedtype(const sort_expression& sort, const std::size_t enumeratedtype_index)
  {
    ...
    const function_symbol_list::const_iterator i = std::find_if(functions.begin(),
      functions.end(),
      [&sort](const data::function_symbol& w)
      {
    ...
    const data::function_symbol casefunction(
      fresh_identifier_generator(
        "C" + std::to_string(n) + "_" + (!is_basic_sort(newsort) ? "" : std::string(basic_sort(sort).name()))),
      newsort);
  ...
  */
  ...
  bool mergeoccursin(variable& var,
    ...
    const variable_list& process_parameters)
  {
    ...
    for (variable_list::const_iterator i = matchinglist.begin(); i != matchinglist.end(); ++i)
    {
      variable var1 = *i;
  ...
  variable_list merge_var(const variable_list& v1,
    ...
    const variable_list& process_parameters)
  {
    ...
    {
      variable v = v_;
      if (!mergeoccursin(v, v2, matchinglist, renamingpars, renamingargs, process_parameters))
      {
        result.push_front(v);
  ...
  variable_list make_binary_sums(std::size_t n,
  ...
  data_expression construct_binary_case_tree_rec(std::size_t n,
  ...
  data_expression construct_binary_case_tree(std::size_t n,
    const variable_list& sums,
    data_expression_list terms,
    const sort_expression& termsort,
    const enumtype& e)
  ...
  static bool summandsCanBeClustered(const stochastic_action_summand& summand1,
  ...
  stochastic_action_summand collect_sum_arg_arg_cond(const enumtype& e,
    std::size_t n,
    ...
    const variable_list& parameters)
  {
    ...
    if (options.binary)
    {
      resultsum = make_binary_sums(n, enumeratedtypes[e.enumeratedtype_index].sortId, binarysumcondition, resultsum);
    }
    else
    {
    ...
    for (const stochastic_action_summand& smmnd: action_summands)
    {
      ...
      for (variable_list::const_iterator i = auxpars.begin(); i != auxpars.end(); ++i, ++j)
      {
    ...
    if (options.binary)
    {
      resultcondition = construct_binary_case_tree(n, resultsum, auxresult, sort_bool::bool_(), e);
      resultcondition = lazy::and_(binarysumcondition, resultcondition);
      resultcondition
        = lazy::and_(construct_binary_case_tree(n, resultsum, conditionlist, sort_bool::bool_(), e), resultcondition);
    }
    else
    {
      if (equaluptillnow)
      {
        ...
        else
        {
          ...
          resultcondition
            = lazy::and_(application(find_case_function(e.enumeratedtype_index, sort_bool::bool_()), tempconditionlist),
              equalterm);
        }
      }
      else
      {
        ...
        resultcondition = application(find_case_function(e.enumeratedtype_index, sort_bool::bool_()), tempauxresult);
        if (all_equal(conditionlist))
        {
    ...
    for (; multiactioncount > 0; multiactioncount--)
    {
      ...
      for (; fcnt > 0; fcnt--)
      {
        ...
        for (stochastic_action_summand_vector::const_iterator walker = action_summands.begin();
          walker != action_summands.end();
        ...
        else
        {
          if (!options.binary)
          {
            ...
            tempauxresult.push_front(data_expression(e.var));
            resultf.push_front(
              data_expression(application(find_case_function(e.enumeratedtype_index, f.sort()), tempauxresult)));
          }
          else
          {
            data_expression temp = construct_binary_case_tree(n, resultsum, auxresult, f.sort(), e);
    ...
    if (some_summand_has_time)
    {
      ...
      if (options.binary == 1)
      {
        resulttime = construct_binary_case_tree(n, resultsum, auxresult, sort_real::real_(), e);
      }
      else
      {
        if (equaluptillnow)
        ...
        else
        {
          data_expression_list tempauxresult = auxresult;
          tempauxresult.push_front(data_expression(e.var));
          resulttime = application(find_case_function(e.enumeratedtype_index, sort_real::real_()), tempauxresult);
        }
      }
    }
    ...
    data_expression resulting_distribution = sort_real::real_one();
    {
      ...
      for (const stochastic_action_summand& smmnd: action_summands)
      {
        ...
        resulting_stochastic_variables = merge_var(stochastic_vars,
          resulting_stochastic_variables,
          stochastic_rename_list_pars,
          stochastic_rename_list_args,
          stochastic_conditionlist,
      ...
      std::vector<data_expression_list>::const_iterator stochastic_auxrename_list_args
        = stochastic_rename_list_args.begin();
      ...
      for (const stochastic_action_summand& smmnd: action_summands)
      {
        ...
        for (variable_list::const_iterator i = stochastic_auxpars.begin(); i != stochastic_auxpars.end(); ++i, ++j)
        {
          /* Substitutions are carried out from left to right. The first applicable substitution counts */
          if (sigma(*i) == *i) // sigma is not defined for *i.
          {
      ...
      if (options.binary)
      {
        resulting_distribution = construct_binary_case_tree(n, resultsum, aux_result, sort_real::real_(), e);
        resulting_distribution
          = lazy::and_(construct_binary_case_tree(n, resultsum, stochastic_conditionlist, sort_bool::bool_(), e),
            resulting_distribution);
      }
      else
      {
        if (equaluptillnow)
        {
          ...
          else
          {
            ...
            resulting_distribution
              = real_times_optimized(if_(application(find_case_function(e.enumeratedtype_index, sort_bool::bool_()),
        ...
        else
        {
          ...
          tempauxresult.push_front(data_expression(e.var));
          resulting_distribution
            = application(find_case_function(e.enumeratedtype_index, sort_real::real_()), tempauxresult);
          if (all_equal(stochastic_conditionlist))
          {
            if (stochastic_conditionlist.front() == sort_bool::true_())
            {
    ...
    for (const variable& var: parameters)
    {
      ...
      for (const stochastic_action_summand& smmnd: action_summands)
      {
        ...
        if (equalterm
            == data_expression()) // ||is_global_variable(equalterm)) Adding this last part leads to smaller case
                                  // functions, but bigger and less structured state spaces, as parameters are less
                                  // often put to default values. Constant elimination can less often be applied as
                                  // constant eliminiation does not have subtle knowledge of case functions.
        {
          equalterm = auxresult1;
        }
      ...
      else
      {
        if (!options.binary)
        {
          ...
          resultnextstate.push_front(
            data_expression(application(find_case_function(e.enumeratedtype_index, var.sort()), tempauxresult)));
        }
        else
        {
          data_expression temp = construct_binary_case_tree(n, resultsum, auxresult, var.sort(), e);
    ...
    return stochastic_action_summand(resultsum,
      resultcondition,
  ...
  deadlock_summand collect_sum_arg_arg_cond(const enumtype& e,
    std::size_t n,
    ...
    const variable_list& parameters)
  {
    ...
    return deadlock_summand(resultsum, resultcondition, some_summand_has_time ? deadlock(resulttime) : deadlock());
  }
  ...
  void cluster_actions(stochastic_action_summand_vector& action_summands,
    deadlock_summand_vector& deadlock_summands,
    const variable_list& pars)
  {
    {
     ...
     from the underlying GNF */
  {
    ...
    */
    ...
    const std::set<process_identifier> stochastic_normalized_process_identifiers
      = remove_stochastic_operators_from_front(reduced_set_of_process_identifiers,
        initial_proc_id,
    ...
    if (regular)
    {
      ...
      else /* !binary or oldstate */
      {
        ...
        parameters = ((!singlecontrolstate) ? tempparameters : stack.parameters);
      }
    }
    ...
    init = make_initialstate(initial_proc_id,
      stack,
      stochastic_normalized_process_identifiers,
      regular,
      singlecontrolstate,
      initial_stochastic_distribution);
    assert(init.size() == parameters.size());
    collectsumlist(action_summands,
      deadlock_summands,
      stochastic_normalized_process_identifiers,
      parameters,
      stack,
  ...
  variable_list construct_renaming(const variable_list& pars1,
  ...
  {
    return sort_expression_list(l.begin(),
      l.end(),
      [](const typename List::value_type& d) -> sort_expression { return d.sort(); });
  }

  static bool check_real_variable_occurrence(const variable_list& sumvars,
  ...
  data_expression makesingleultimatedelaycondition(const variable_list& sumvars,
    const variable_list& freevars,
    const data_expression& condition,
    const bool has_time,
    ...
    variable_list& used_sumvars)
  {
    ...
    */
    ...
    {
      result = RewriteTerm(lazy::and_(condition, data::less(timevariable, actiontime)));
  ...
  lps::detail::ultimate_delay getUltimateDelayCondition(const stochastic_action_summand_vector& action_summands,
    const deadlock_summand_vector& deadlock_summands,
    const variable_list& freevars)
  {
    ...
    for (const deadlock_summand& s: deadlock_summands)
    {
      ...
      const data_expression ult_del_condition = makesingleultimatedelaycondition(s.summation_variables(),
        freevars,
        s.condition(),
        s.deadlock().has_time(),
        time_variable,
      ...
      existentially_quantified_variables = merge_var(new_existentially_quantified_variables,
    ...
    for (data_expression_list::const_iterator i = results.begin(); i != results.end();
      ++i, ++j, ++renamings_par, ++renamings_arg)
    {
      ...
      for (variable_list::const_iterator i1 = auxpars.begin(); i1 != auxpars.end(); ++i1, ++j1)
      {
        /* Substitutions are carried out from left to right. The first applicable substitution counts */
        if (sigma(*i1) == *i1)
        {
  ...
  data::maintain_variables_in_rhs<data::mutable_map_substitution<>> make_unique_variables(const variable_list& var_list,
  ...
  void make_parameters_and_sum_variables_unique(stochastic_action_summand_vector& action_summands,
    deadlock_summand_vector& deadlock_summands,
    variable_list& pars,
    ...
    const std::string& hint = "")
  {
    ...
    {
      const variable_list& sumvars = smmnd.summation_variables();
      data::maintain_variables_in_rhs<data::mutable_map_substitution<>> sigma_sumvars
        = make_unique_variables(sumvars, hint);
      ...
      multiaction = lps::replace_variables_capture_avoiding_with_an_identifier_generator(multiaction,
        sigma_sumvars,
        fresh_identifier_generator);
      multiaction = lps::replace_variables_capture_avoiding_with_an_identifier_generator(multiaction,
        sigma,
      ...
      distribution = stochastic_distribution(unique_stochastic_vars,
  ...
  lps::detail::ultimate_delay combine_ultimate_delays(const lps::detail::ultimate_delay& delay1,
    const lps::detail::ultimate_delay& delay2)
  {
    ...
    return lps::detail::ultimate_delay(delay1.time_var(), new_existential_variables, new_constraint);
  }

  void calculate_left_merge_action(const lps::detail::ultimate_delay& ultimate_delay_condition,
  ...
  void calculate_left_merge_deadlock(const lps::detail::ultimate_delay& ultimate_delay_condition,
    ...
    size_t indent = 0)
  {
    ...
    os << indent_str << "LPS before:" << std::endl
       << print(lps_statistics_before, indent + 2) << indent_str << "result before:" << std::endl
       << print(lps_statistics_result_before, indent + 2) << indent_str << "result after:" << std::endl
  ...
  void calculate_left_merge(const stochastic_action_summand_vector& action_summands1,
    const deadlock_summand_vector& deadlock_summands1,
    ...
    deadlock_summand_vector& deadlock_summands)
  {
    ...
    calculate_left_merge_deadlock(ultimate_delay_condition2,
      deadlock_summands1,
      is_allow,
      is_block,
      action_summands,
      deadlock_summands);
    calculate_left_merge_action(ultimate_delay_condition2,
      action_summands1,
      allowlist,
      is_allow,
      is_block,
    ...
    std::cout << log_leftmerge_application(lps_statistics_before,
      ...
      is_allow,
      is_block,
      (is_block ? allowlist.front().size() : allowlist.size()),
      4);
#endif
  }

  void calculate_communication_merge_action_summands(const stochastic_action_summand_vector& action_summands1,
        ...
        if ((multiaction1 == action_list({terminationAction})) == (multiaction2 == action_list({terminationAction})))
        {
          ...
          else
          {
            ...
            else
            {
              ...
              action_time3 = actiontime1;
              condition3 = lazy::and_(condition3, equal_to(actiontime1, actiontime2));
            }
          }

          const assignment_list nextstate3 = nextstate1 + nextstate2;
          const stochastic_distribution distribution3(distribution1.variables() + distribution2.variables(),
  ...
  void calculate_communication_merge_action_deadlock_summands(const stochastic_action_summand_vector& action_summands1,
    const deadlock_summand_vector& deadlock_summands1,
    const stochastic_action_summand_vector& action_summands,
    deadlock_summand_vector& deadlock_summands)
  {
    for (const stochastic_action_summand& summand1: action_summands1)
    {
      ...
      for (const deadlock_summand& summand2: deadlock_summands1)
      {
        ...
        if (condition3 != sort_bool::false_() && !options.ignore_time)
        {
          insert_timed_delta_summand(action_summands,
            deadlock_summands,
            deadlock_summand(allsums, condition3, has_time3 ? deadlock(action_time3) : deadlock()),
            options.ignore_time);
        }
      }
    }
  ...
  void calculate_communication_merge_deadlock_summands(const deadlock_summand_vector& deadlock_summands1,
    ...
    size_t indent = 0)
  {
    ...
    os << indent_str << "first LPS before:" << std::endl
       << print(lps1_statistics_before, indent + 2) << indent_str << "second LPS before:" << std::endl
       << print(lps2_statistics_before, indent + 2) << indent_str << "result LPS before:" << std::endl
       << print(lps_result_statistics_before, indent + 2) << indent_str << "result LPS after:" << std::endl
       << print(lps_statistics_after, indent + 2);
  ...
  void calculate_communication_merge(const stochastic_action_summand_vector& action_summands1,
    const deadlock_summand_vector& deadlock_summands1,
    const stochastic_action_summand_vector& action_summands2,
    const deadlock_summand_vector& deadlock_summands2,
    const action_name_multiset_list& allowlist, // This is a list of list of identifierstring.
    ...
    deadlock_summand_vector& deadlock_summands)
  {
    ...
    calculate_communication_merge_action_summands(action_summands1,
      action_summands2,
      allowlist,
      is_allow,
      is_block,
      action_summands);
    calculate_communication_merge_action_deadlock_summands(action_summands1,
      deadlock_summands2,
      action_summands,
      deadlock_summands);
    calculate_communication_merge_action_deadlock_summands(action_summands2,
      deadlock_summands1,
      action_summands,
      deadlock_summands);
    calculate_communication_merge_deadlock_summands(deadlock_summands1,
      deadlock_summands2,
      action_summands,
      deadlock_summands);
    ...
    std::cout << log_communicationmerge_application(lps1_statistics_before,
      lps2_statistics_before,
      lps_result_statistics_before,
  ...
  void combine_summand_lists(const stochastic_action_summand_vector& action_summands1,
    const deadlock_summand_vector& deadlock_summands1,
    ...
    deadlock_summand_vector& deadlock_summands)
  {
    ...
    calculate_left_merge(action_summands1,
      deadlock_summands1,
      ultimate_delay_condition2,
      allowlist,
      is_allow,
      is_block,
      action_summands,
    ...
    calculate_left_merge(action_summands2,
      deadlock_summands2,
      ultimate_delay_condition1,
      allowlist,
      is_allow,
      is_block,
      action_summands,
    ...
    calculate_communication_merge(action_summands1,
      deadlock_summands1,
      action_summands2,
      deadlock_summands2,
      allowlist,
...
#ifdef MCRL2_LOG_LPS_LINEARISE_STATISTICS
  static std::string log_parallelcomposition_application_start(const lps_statistics_t& lps1_statistics_before,
    const lps_statistics_t& lps2_statistics_before,
    const bool is_allow,
    const bool is_block,
    const std::size_t num_allow_block_actions,
    size_t indent = 0)
  {
    ...
    os << indent_str << "first LPS before:" << std::endl
       << print(lps1_statistics_before, indent + 2) << indent_str << "second LPS before:" << std::endl
       << print(lps2_statistics_before, indent + 2);
  ...
  static std::string log_parallelcomposition_application_end(const lps_statistics_t& lps_statistics_after,
    size_t indent = 0)
  {
    indent += 2;
    std::string indent_str(indent, ' ');
  ...
  void parallelcomposition(const stochastic_action_summand_vector& action_summands1,
    const deadlock_summand_vector& deadlock_summands1,
    ...
    lps::detail::ultimate_delay& ultimate_delay_condition)
  {
    mCRL2log(mcrl2::log::verbose) << (is_allow   ? "- calculating the parallel composition modulo the allow operator: "
                                      : is_block ? "- calculating the parallel composition modulo the block operator: "
                                                 : "- calculating the parallel composition: ")
                                  << action_summands1.size() << " action summands + " << deadlock_summands1.size()
                                  << " deadlock summands || " << action_summands2.size() << " action summands + "
                                  << deadlock_summands2.size() << " deadlock summands = ";
    ...
    std::cout << log_parallelcomposition_application_start(lps1_statistics_before,
      lps2_statistics_before,
      is_allow,
      is_block,
    ...
    assert(action_summands.size() == 0);
    assert(deadlock_summands.size() == 0);
    combine_summand_lists(action_summands1,
      deadlock_summands1,
      ultimate_delay_condition1,
      action_summands2,
      deadlock_summands2,
    ...
    mCRL2log(mcrl2::log::verbose) << action_summands.size() << " actions and " << deadlock_summands.size()
                                  << " delta summands.\n";
    pars_result = pars1 + pars3;
    init_result = init1 + init2;
    initial_stochastic_distribution = stochastic_distribution(initial_stochastic_distribution1.variables()
                                                                + initial_stochastic_distribution2.variables(),
  ...
  void generateLPEmCRLterm(stochastic_action_summand_vector& action_summands,
    deadlock_summand_vector& deadlock_summands,
    const process_expression& t,
    const bool regular,
    ...
    lps::detail::ultimate_delay& ultimate_delay_condition)
  {
    if (is_process_instance_assignment(t))
    {
      generateLPEmCRL(action_summands,
        deadlock_summands,
        process_instance_assignment(t).identifier(),
        regular,
        pars,
        init,
        initial_stochastic_distribution,
        ultimate_delay_condition);
      ...
      initial_stochastic_distribution = stochastic_distribution(initial_stochastic_distribution.variables(),
      ...
      if ((object.processstatus == GNF) || (object.processstatus == pCRL) || (object.processstatus == GNFalpha))
      {
        make_parameters_and_sum_variables_unique(action_summands,
          deadlock_summands,
          pars,
          ultimate_delay_condition,
      ...
      if (regular && !options.do_not_apply_constelm)
      {
        ...
        const std::set<variable> variable_list1
          = lps::find_free_variables(temporary_spec.process().deadlock_summands());
        ...
        for (const variable& v: variable_list)
        {
          if (std::find(pars.begin(), pars.end(), v) == pars.end() && // The free variable is not in pars,
            ...
            // (lps::search_free_variable(temporary_spec.process().action_summands(),v) ||
            // lps::search_free_variable(temporary_spec.process().deadlock_summands(),v))
    ...
    if (is_merge(t))
    {
      ...
      generateLPEmCRLterm(action_summands1,
        deadlock_summands1,
        process::merge(t).left(),
        regular,
        rename_variables,
        pars1,
        init1,
        initial_stochastic_distribution1,
        ultimate_delay_condition1);
      generateLPEmCRLterm(action_summands2,
        deadlock_summands2,
        process::merge(t).right(),
        regular,
        true,
        pars2,
        init2,
        initial_stochastic_distribution2,
        ultimate_delay_condition2);
      parallelcomposition(action_summands1,
        deadlock_summands1,
        pars1,
        init1,
        initial_stochastic_distribution1,
    ...
    if (is_hide(t))
    {
      generateLPEmCRLterm(action_summands,
        deadlock_summands,
        hide(t).operand(),
        regular,
        rename_variables,
    ...
    if (is_allow(t))
    {
      ...
      else if (!options.nodeltaelimination && options.ignore_time && is_comm(par))
      {
        generateLPEmCRLterm(action_summands,
          ...
          initial_stochastic_distribution,
          ultimate_delay_condition);
        communicationcomposition(comm(par).comm_set(),
          allow(t).allow_set(),
          true,
          false,
          action_summands,
          ...
          terminationAction,
          options.nosumelm,
          options.nodeltaelimination,
          options.ignore_time,
          [this](const data::data_expression& e) { return RewriteTerm(e); });
        return;
      }
      ...
      generateLPEmCRLterm(action_summands,
        ...
        initial_stochastic_distribution,
        ultimate_delay_condition);
      allowblockcomposition(allow(t).allow_set(),
        true,
        action_summands,
        deadlock_summands,
        terminationAction,
    ...
    if (is_block(t))
    {
      ...
      if (!options.nodeltaelimination && options.ignore_time && is_merge(par))
      {
        ...
        parallelcomposition(action_summands1,
          ...
          initial_stochastic_distribution2,
          ultimate_delay_condition2,
          action_name_multiset_list({action_name_multiset(block(t).block_set())}),
          false,
          true,
          action_summands,
          deadlock_summands,
      ...
      generateLPEmCRLterm(action_summands,
        ...
        ultimate_delay_condition);
      // Encode the actions of the block list in one multi action.
      allowblockcomposition(action_name_multiset_list({action_name_multiset(block(t).block_set())}),
        false,
        action_summands,
        deadlock_summands,
        terminationAction,
    ...
    if (is_comm(t))
    {
      generateLPEmCRLterm(action_summands,
        ...
        initial_stochastic_distribution,
        ultimate_delay_condition);
      communicationcomposition(comm(t).comm_set(),
        action_name_multiset_list(),
        false,
        false,
        action_summands,
    ...
    if (is_stochastic_operator(t))
    {
      ...
      initial_stochastic_distribution = stochastic_distribution(stochvars + initial_stochastic_distribution.variables(),
        sort_real::times(replace_variables_capture_avoiding_alt(sto.distribution(), sigma),
          initial_stochastic_distribution.distribution()));
      /* Reset the bound variables in the initial_stochastic_distribution, to avoid erroneous renaming in the body of
       * the process */
  ...
  */
  ...
  void generateLPEmCRL(stochastic_action_summand_vector& action_summands,
    deadlock_summand_vector& deadlock_summands,
    ...
    lps::detail::ultimate_delay& ultimate_delay_condition)
  {
    ...
    if ((object.processstatus == GNF) || (object.processstatus == pCRL) || (object.processstatus == GNFalpha)
        || (object.processstatus == multiAction))
    {
      generateLPEpCRL(action_summands,
        deadlock_summands,
        procIdDecl,
        object.containstime,
        regular,
    ...
    if ((object.processstatus == mCRLdone) || (object.processstatus == mCRLlin) || (object.processstatus == mCRL))
    {
  ...
  process_expression alphaconversionterm(const process_expression& t,
    const variable_list& parameters,
    maintain_variables_in_rhs<mutable_map_substitution<>> sigma)
  {
    if (is_choice(t))
    {
      return choice(alphaconversionterm(choice(t).left(), parameters, sigma),
        alphaconversionterm(choice(t).right(), parameters, sigma));
    }

    if (is_seq(t))
    {
      return seq(alphaconversionterm(seq(t).left(), parameters, sigma),
        alphaconversionterm(seq(t).right(), parameters, sigma));
    }

    if (is_sync(t))
    {
      return process::sync(alphaconversionterm(process::sync(t).left(), parameters, sigma),
        alphaconversionterm(process::sync(t).right(), parameters, sigma));
    }

    if (is_bounded_init(t))
    {
      return bounded_init(alphaconversionterm(bounded_init(t).left(), parameters, sigma),
        alphaconversionterm(bounded_init(t).right(), parameters, sigma));
    }

    if (is_merge(t))
    {
      return process::merge(alphaconversionterm(process::merge(t).left(), parameters, sigma),
        alphaconversionterm(process::merge(t).right(), parameters, sigma));
    }

    if (is_left_merge(t))
    {
      return left_merge(alphaconversionterm(left_merge(t).left(), parameters, sigma),
    ...
    if (is_if_then(t))
    {
      return if_then(replace_variables_capture_avoiding_alt(if_then(t).condition(), sigma),
  ...
  void alphaconversion(const process_identifier& procId, const variable_list& parameters)
  {
    ...
    if ((object.processstatus == GNF) || (object.processstatus == multiAction))
    {
    ...
    {
      throw mcrl2::runtime_error(
        "unknown type " + std::to_string(object.processstatus) + " in alphaconversion of " + process::pp(procId) + ".");
    }
    return;
  }
    ...
    bool& contains_if_then)
  {
    ...
    if (is_action(t) || is_delta(t) || is_tau(t))
    {
      return false;
    }
  ...
  bool containstime_rec(const process_identifier& procId,
    bool* stable,
    std::set<process_identifier>& visited,
    bool& contains_if_then)
  {
    ...
    if (visited.count(procId) == 0)
    {
      ...
      {
        mCRL2log(mcrl2::log::warning)
          << "process " << procId.name() << " contains time, which is now not preserved. \n"
...
public:
  ...
  process_expression transform_initial_distribution_term(const process_expression& t,
    const std::map<process_identifier, process_pid_pair>& processes_with_initial_distribution)
  {
    if (is_process_instance_assignment(t))
    {
      ...
      if (is_stochastic_operator(new_process))
      {
        ...
        const variable_list relevant_stochastic_variables
          = parameters_that_occur_in_body(sto.variables(), object.processbody);
    ...
    if (is_choice(t))
    {
      const process_expression r1_
        = transform_initial_distribution_term(choice(t).left(), processes_with_initial_distribution);
      const process_expression r2_
        = transform_initial_distribution_term(choice(t).right(), processes_with_initial_distribution);
      if (is_stochastic_operator(r1_))
      {
        ...
        if (is_stochastic_operator(r2_))
        {
          ...
          const data_expression new_distribution
            = process::replace_variables_capture_avoiding_with_an_identifier_generator(r2.distribution(),
              local_sigma,
              fresh_identifier_generator);
    ...
    if (is_seq(t))
    {
      const process_expression r1
        = transform_initial_distribution_term(seq(t).left(), processes_with_initial_distribution);
      const process_expression r2
        = transform_initial_distribution_term(seq(t).right(), processes_with_initial_distribution);
    ...
    if (is_if_then(t))
    {
      ...
      const process_expression r
        = transform_initial_distribution_term(t_.then_case(), processes_with_initial_distribution);
      if (is_stochastic_operator(r))
      {
        ...
        return stochastic_operator(r_.variables(),
          if_(t_.condition(),
            r_.distribution(),
            if_(variables_are_equal_to_default_values(r_.variables()), sort_real::real_one(), sort_real::real_zero())),
    ...
    if (is_if_then_else(t))
    {
      ...
      const process_expression r1_
        = transform_initial_distribution_term(t_.then_case(), processes_with_initial_distribution);
      const process_expression r2_
        = transform_initial_distribution_term(t_.else_case(), processes_with_initial_distribution);
      if (is_stochastic_operator(r1_))
      {
        const stochastic_operator& r1 = down_cast<const stochastic_operator>(r1_);
        if (is_stochastic_operator(r2_))
    ...
    if (is_sum(t))
    {
      ...
      const process_expression r_
        = transform_initial_distribution_term(s.operand(), processes_with_initial_distribution);
      if (is_stochastic_operator(r_))
      {
        ...
        for (const variable& v: s.variables())
        {
          ...
          {
            throw mcrl2::runtime_error("Cannot commute a sum operator over a stochastic operator in " + process::pp(t)
                                       + ".\n" + "The problematic variable is " + pp(v) + ":" + pp(v.sort()) + ".");
          }
        }
        return stochastic_operator(r.variables(), r.distribution(), sum(s.variables(), r.operand()));
      }
    ...
    if (is_stochastic_operator(t))
    {
      const stochastic_operator& sto = down_cast<const stochastic_operator>(t);
      const process_expression r_
        = transform_initial_distribution_term(sto.operand(), processes_with_initial_distribution);
    ...
    {
      const process_expression r_
        = transform_initial_distribution_term(at(t).operand(), processes_with_initial_distribution);
      if (is_stochastic_operator(r_))
      {
        const stochastic_operator& r = down_cast<const stochastic_operator>(r_);
    ...
    throw mcrl2::runtime_error(
      "unexpected process format in transform_initial_distribution_term " + process::pp(t) + ".");
  }
    ...
    if (is_if_then(t))
    {
      ...
      if (is_stochastic_operator(r_))
      {
        ...
        return stochastic_operator(r.variables(),
          if_(t_if_then.condition(),
            r.distribution(),
            if_(variables_are_equal_to_default_values(r.variables()), sort_real::real_one(), sort_real::real_zero())),
  ...
  process_expression obtain_initial_distribution(const process_identifier& procId)
  {
    ...
    {
      const process_identifier new_procId = newprocess(object.parameters + initial_distribution.variables(),
        initial_distribution.operand(),
        pCRL,
        canterminatebody(initial_distribution.operand()),
        containstimebody(initial_distribution.operand()));
...
public:
    ...
    std::map<process_expression, process_expression>& visited_proc)
  {
    ...
    if ((object.processstatus != mCRL) && (object.canterminate == 0))
    {
    ...
    if (object.processstatus == mCRL)
    {
      ...
      object.processbody = split_body(object.processbody, visited_id, visited_proc, object.parameters);
      return procId;
    }

    const process_identifier newProcId(fresh_identifier_generator(procId.name()), object.parameters);
    visited_id[procId] = newProcId;

    if (object.canterminate)
    {
      assert(check_valid_process_instance_assignment(terminatedProcId, assignment_list()));
      insert_process_declaration(newProcId,
        object.parameters,
        seq(object.processbody, process_instance_assignment(terminatedProcId, assignment_list())),
        pCRL,
        canterminatebody(object.processbody),
    ...
    std::set<process_identifier>& visited_processes)
  {
    ...
    {
      const process_instance_assignment q
        = expand_process_instance_assignment(down_cast<process_instance_assignment>(object.processbody),
          visited_processes);
  ...
  void transform_process_arguments(const process_identifier& procId, std::set<process_identifier>& visited_processes)
  {
    if (visited_processes.count(procId) == 0)
    {
      ...
      object.processbody = transform_process_arguments_body(object.processbody, bound_variables, visited_processes);
    }
  }
...
public:
  ...
  process_expression transform_process_arguments_body(const process_expression& t,
    const std::set<variable>& bound_variables,
    std::set<process_identifier>& visited_processes)
  {
    if (is_process_instance(t))
    {
      ...
      process_instance_assignment t1
        = transform_process_instance_to_process_instance_assignment(atermpp::down_cast<process_instance>(t),
          bound_variables);
    ...
    if (is_process_instance_assignment(t))
    {
      ...
      assert(
        check_valid_process_instance_assignment(u.identifier(), sort_assignments(u.assignments(), object.parameters)));
      process_instance_assignment t1(u.identifier(), sort_assignments(u.assignments(), object.parameters));
      return expand_process_instance_assignment(t1);
    }
    if (is_hide(t))
    {
    ...
    if (is_rename(t))
    {
      return process::rename(process::rename(t).rename_set(),
    ...
    if (is_merge(t))
    {
      return merge(transform_process_arguments_body(merge(t).left(), bound_variables, visited_processes),
        transform_process_arguments_body(merge(t).right(), bound_variables, visited_processes));
    }
    if (is_choice(t))
    {
      return choice(transform_process_arguments_body(choice(t).left(), bound_variables, visited_processes),
        transform_process_arguments_body(choice(t).right(), bound_variables, visited_processes));
    }
    if (is_seq(t))
    {
      return seq(transform_process_arguments_body(seq(t).left(), bound_variables, visited_processes),
        transform_process_arguments_body(seq(t).right(), bound_variables, visited_processes));
    }
    if (is_if_then_else(t))
    {
      return if_then_else(if_then_else(t).condition(),
    ...
    if (is_sum(t))
    {
      ...
      return sum(sum_vars, transform_process_arguments_body(sum(t).operand(), bound_variables1, visited_processes));
    }
    if (is_action(t))
    {
      return t;
    ...
    if (is_at(t))
    {
      return at(transform_process_arguments_body(at(t).operand(), bound_variables, visited_processes),
        at(t).time_stamp());
    }
    if (is_sync(t))
    {
    ...
    if (is_stochastic_operator(t))
    {
      ...
      return stochastic_operator(vars,
        dist,
  ...
  process_expression split_body(const process_expression& t,
    std::map<process_identifier, process_identifier>& visited_id,
    std::map<process_expression, process_expression>& visited_proc,
    const variable_list& parameters)
  {
    ...
    if (is_merge(t))
    {
      result = process::merge(split_body(process::merge(t).left(), visited_id, visited_proc, parameters),
    ...
    else if (is_process_instance_assignment(t))
    {
      ...
      result = process_instance_assignment(split_process(u.identifier(), visited_id, visited_proc),
        sort_assignments(u.assignments(), object.parameters));
    }
    else if (is_hide(t))
    {
      result = hide(hide(t).hide_set(), split_body(hide(t).operand(), visited_id, visited_proc, parameters));
    }
    else if (is_rename(t))
    {
      result = process::rename(process::rename(t).rename_set(),
        split_body(process::rename(t).operand(), visited_id, visited_proc, parameters));
    }
    else if (is_allow(t))
    {
      result = allow(allow(t).allow_set(), split_body(allow(t).operand(), visited_id, visited_proc, parameters));
    }
    else if (is_block(t))
    {
      result = block(block(t).block_set(), split_body(block(t).operand(), visited_id, visited_proc, parameters));
    }
    else if (is_comm(t))
    {
      result = comm(comm(t).comm_set(), split_body(comm(t).operand(), visited_id, visited_proc, parameters));
    }
    ...
    else if (is_choice(t) || is_seq(t) || is_if_then_else(t) || is_if_then(t) || is_sum(t) || is_action(t)
             || is_delta(t) || is_tau(t) || is_at(t) || is_sync(t))
    {
  ...
  void AddTerminationActionIfNecessary(const stochastic_action_summand_vector& summands)
  {
    for (const stochastic_action_summand& smd: summands)
    {
      ...
      if (multiaction == action_list({terminationAction}))
      {
        ...
        mCRL2log(mcrl2::log::warning)
          << "The action " << process::pp(terminationAction)
...
public:
  ...
  variable_list SieveProcDataVarsSummands(const std::set<variable>& vars,
    const stochastic_action_summand_vector& action_summands,
    const deadlock_summand_vector& deadlock_summands,
    const variable_list& parameters)
  {
    ...
    variable_list result;
    for (std::set<variable>::reverse_iterator i = vars_result_set.rbegin(); i != vars_result_set.rend(); ++i)
    {
      result.push_front(*i);
    }
...
public:
  variable_list SieveProcDataVarsAssignments(const std::set<variable>& vars,
  ...
  /**************** transform **************************************/
public:
  void transform(const process_identifier& init,
    stochastic_action_summand_vector& action_summands,
    deadlock_summand_vector& deadlock_summands,
    variable_list& parameters,
    ...
    stochastic_distribution& initial_stochastic_distribution)
  {
    ...
    {
      throw mcrl2::runtime_error("There are no processes to be linearised. This is most likely due to the use of "
                                 "unguarded recursion in process equations");
    ...
    allowblockcomposition(action_name_multiset_list({action_name_multiset()}),
      ...
      terminationAction,
      options.ignore_time,
      options.nodeltaelimination); // This removes superfluous delta summands.
...
public:
  ...
  bool run() override
  {
    ...
    auto init_expr = spec.init();


    mcrl2::lps::t_lin_options lin_options;

    // mcrl2::lps::collectPcrlProcesses(spec.init(), pcrlprocesslist);
    specification_basic_type1 basic_spec(spec.action_labels(),
      spec.equations(),
      data::variable_list(spec.global_variables().begin(), spec.global_variables().end()),
      spec.data(),
      spec.global_variables(),
      lin_options,
      spec);
    process_identifier init = basic_spec.storeinit(spec.init());

    basic_spec.transform_process_arguments(init); // Also merges nested stochastic operators.
    basic_spec.determine_process_status(init, mCRL);
    basic_spec.determinewhetherprocessescanterminate(init);
    const process_identifier init_ = basic_spec.splitmCRLandpCRLprocsAndAddTerminatedAction(init);
    basic_spec.determinewhetherprocessescontaintime(init_);
    std::vector<mcrl2::process::process_identifier> pcrlprocesslist;


    // basic_spec.collectPcrlProcesses(init_, pcrlprocesslist);
    // basic_spec.transform(const process_identifier &init, stochastic_action_summand_vector &action_summands, deadlock_summand_vector &deadlock_summands, variable_list &parameters, data_expression_list &initial_state, stochastic_distribution &initial_stochastic_distribution)

    // for (auto id: pcrlprocesslist)
    // {
    //   std::cout << id.name() << "\n";
    // }
   
    variable_list vars1(basic_spec.global_variables.begin(), basic_spec.global_variables.end());
    action_label_list acts1(basic_spec.acts.begin(), basic_spec.acts.end());
    process_equation_list eqs1(basic_spec.procs.begin(), basic_spec.procs.end());
    mcrl2::process::process_specification spec1(
      basic_spec.data,
      acts1,
      vars1,
      eqs1,
      process_expression(init_) 
    );

    std::cout << mcrl2::process::pp(spec1, false) << std::endl;
    return true;
  }
};
